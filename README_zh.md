...
> 该指令用于指定获取窗口信息的方式，请注意潜在的隐私安全问题。
> 
> 默认为窗口名称获取，此时Petto将把当前窗口的标题作为信息传递给语言模型。
> 
> 需要注意的是，如果您把“获取窗口信息命令”设置为空，无论选择任何模式，Petto都将不会获取任何窗口信息。

Petto默认提供了`getwindowname.ps1`和`getscreenshot.ps1`两种获取脚本，前者获取窗口名称，后者获取窗口截图。

对于Windows用户，如果您将选项修改为截图获取，您需要把“获取窗口信息命令”中的`getwindowname.ps1`修改为`getscreenshot.ps1`。

默认提供的脚本仅支持Windows，如果您使用macOS，您需要自行编写获取窗口信息的脚本，并修改“获取窗口信息命令”的值。
